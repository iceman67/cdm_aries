...
class AriesAgent(DemoAgent):
    ...
    # new method
    async def handle_encrypt(self, message):
        pass
    ...
    def add_op(self, obj):
        ...
        self.log("sum message:", result) 

    # {"op":"-","value":[10, 20, 30]}
    def subtract_op(self, obj):
        operand = obj["value"]
        result = reduce(lambda x, y: x - y, operand)
        self.log("subtract message:", result) 

    # {"op" :"encypt", "value" : "test"}
    def encrypt_op(self, obj):
        operand = obj["value"]
        result = operand + "**"
        self.log("encrypt message:", result) 
    ...
    async def handle_basicmessages(self, message):
        ...
        if "op" in message["content"]:
            ...
            if obj["op"] == "+":
                ...
                # {"op":"-","value":[10, 20, 30]}
                self.subtract_op(obj)
                # {"op" :"encrypt", "value" : "test"}
            elif obj["op"] == "encrypt":
                self.encrypt_op(obj)
        else: 
