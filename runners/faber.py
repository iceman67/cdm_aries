...
async def main(args):
    ...
    try:
        ...
        if faber_agent.cred_type == CRED_FORMAT_INDY:
            ...
            if schema_domain == 'CDM':
                faber_schema_name = "CDM schema"
                faber_schema_attrs = ["name", "date", "degree", "age", "key", "iv", "timestamp"]
        ...
        async for option in prompt_loop(options):
            ...
            if option is None or option in "xX":
                ...
                log_status("#20 Request proof of degree from alice researcher")
                
                if faber_agent.aip == 10:
                    req_attrs = [
                        {
                            "name": "name",
                            "restrictions": [{"schema_name": "CDM schema"}],
                        },
                        {
                            "name": "date",
                            "restrictions": [{"schema_name": "CDM schema"}],
